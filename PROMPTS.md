...
This document provides comprehensive details about the 6 core AI documentation prompts implemented in the GitBook MCP server. These prompts are designed to facilitate essential documentation workflows through conversational AI interfaces and work seamlessly with the 7 available MCP tools (read-only fork — see `README.md`).

> Note: `troubleshooting_assistant`, documented below as prompt 6, was removed from the source in a prior commit but the doc entry was never deleted — it is **not** a live prompt. The actually-shipped 6th prompt is `propose_page_edit` (§7 below).
...
**Related MCP Tools**: `get_space_content`, `get_page_content`, `get_space`
...
**Related MCP Tools**: `get_space_content`, `get_page_content`, `get_space`, `list_organizations`
...
**Related MCP Tools**: `get_page_content`, `search_content`, `get_space_content`
...
**Related MCP Tools**: `get_space`, `get_page_by_path`, `list_organizations`, `list_spaces`
...
**Purpose**: Draft a create/update proposal for a GitBook page for human review — never writes to GitBook, there is no write tool in this server (see `DESIGN.md` §9–10)

**Parameters**:
- `spaceId` (optional): The GitBook space ID to target (uses default space if not provided)
- `pagePath` (optional): Path of the existing page to update, or intended path for a new page. Omit if undecided.
- `changeDescription` (required): What should change, e.g. "VPN IPs changed, update with new values" or "document the new staging deploy process"

**Related MCP Tools**: `get_page_by_path`, `get_space_content`

**Example Usage**:
```
Use the propose_page_edit prompt with:
- spaceId: "abc123"
- pagePath: "guides/vpn-setup"
- changeDescription: "VPN IPs changed, update with new values"
```

**What it does**: Checks whether the target page exists via `get_page_by_path` to branch into update vs. create mode, drafts markdown content against the requested change (plus a change summary, or a proposed placement for new pages via `get_space_content`), and returns the draft with an explicit "Not applied — paste this into GitBook yourself" closing line. Refuses outright for any space ID in `GITBOOK_EXCLUDED_SPACE_IDS` (defaults to the public docs space).
...
- **get_space_content** → `fetch_documentation`, `analyze_content_gaps`, `content_audit`, `documentation_summary`, `propose_page_edit`

### Page Content Tools  
- **get_page_content** → `fetch_documentation`, `content_optimization`, `content_audit`
- **get_page_by_path** → `fetch_documentation`, `troubleshooting_assistant`, `propose_page_edit`
